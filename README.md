...
> ⚠️ **重要限制（请务必知悉）**：GitHub runner 使用**数据中心 IP**，Cloudflare 对机房 IP 信誉评分低，
> 云端在**需要登录时很可能过不了 Turnstile**。工作流虽用 `actions/cache` 复用 `.profile/`，但这份缓存是
> 【云端自己】从首次成功运行开始积累的——本地 `.profile/` 不进仓库，**无法直接喂给云端**。
> 换言之：只要云端**成功登录过一次**，之后即可靠缓存登录态自动签到；但若云端**从未登录成功**，
> 缓存建立不起来便会持续失败。若云端长期失败，请改用上文的**本机 cron / 家用设备**运行，家庭宽带 IP 成功率高得多。
